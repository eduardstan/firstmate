...
secondmate_liveness_one() {  # <meta> <id>
  ...
  case "$harness" in
    claude|codex|opencode|pi|pi-signed|prime-agent|grok|kimi|omp) ;;
    *)
...
crew_dispatch_validate() {
  ...
  err=$(jq -r '
    def verified($h): ["claude","codex","opencode","pi","pi-signed","prime-agent","grok","kimi","cursor","gemini","muse","rovo","omp"] | index($h);
    def effort_ok($h; $e):
      if $e == null then true
