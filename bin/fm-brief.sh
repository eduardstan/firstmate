...
# sections when the task genuinely deviates (e.g. working an existing external PR
...
Optional helper: \`bin/fm-secondmate-report.sh --handled $INBOX_DIR/handled/NNN.msg <verb> <note>\` reads the exact correlation token from the acknowledged request; the legacy \`<verb> <corr_id> <note>\` form also works. The helper appends the correlated line to the parent channel itself - do not pass a status path, and do not write a hand path under this home.
