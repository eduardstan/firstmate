...
cleanup_firstmate_home_children() {
  ...
  for child_meta in "$sub_state"/*.meta; do
    ...
    rm -f "$sub_state/$child_id.turn-ended" \
      "$sub_state/$child_id.pi-ext.ts" "$sub_state/$child_id.prime-ext.ts" "$sub_state/$child_id.omp-ext.ts" \
...
rm -f "$STATE/$ID.turn-ended" \
  "$STATE/$ID.pi-ext.ts" "$STATE/$ID.prime-ext.ts" "$STATE/$ID.omp-ext.ts" "$STATE/$ID.grok-turnend-token" \
