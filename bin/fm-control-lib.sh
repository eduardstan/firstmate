...
fm_control_harness_supported() {  # <harness>
  case "${1-}" in
    claude|codex|opencode|pi|pi-signed|prime-agent|grok|kimi|cursor|gemini|muse|rovo) return 0 ;;
  esac
  return 1
}
...
# and friends. This is the one place that prefix rule is stated. `pi`,
# `pi-signed`, and `prime-agent` are exact because their verified executable
# identities are exact and a prefix would accept an unmeasured variant; an
...
fm_control_interrupt_key() {  # <harness>
  case "${1-}" in
    claude|codex|opencode|pi|pi-signed|prime-agent|kimi|cursor|gemini|muse|rovo) printf 'Escape' ;;
    grok) printf 'C-c' ;;
    *) return 1 ;;
  esac
}
...
fm_control_interrupt_repeat() {  # <harness>
  case "${1-}" in
    opencode) printf '2' ;;
    claude|codex|pi|pi-signed|prime-agent|grok|kimi|cursor|gemini|muse|rovo) printf '1' ;;
    *) return 1 ;;
  esac
}
...
fm_control_interrupt_clear_key() {  # <harness>
  case "${1-}" in
    muse) printf 'C-u' ;;
    claude|codex|opencode|pi|pi-signed|prime-agent|grok|kimi|cursor|gemini|rovo) ;;
    *) return 1 ;;
  esac
}
...
fm_control_interrupt_ack_source() {  # <harness>
  case "${1-}" in
    ...
    claude|codex|opencode|pi|pi-signed|prime-agent|grok|kimi|cursor|gemini|rovo) printf 'none' ;;
    *) return 1 ;;
  esac
}
...
fm_control_exit_command() {  # <harness>
  case "${1-}" in
    ...
    codex|pi|pi-signed|prime-agent|gemini) printf '/quit' ;;
    *) return 1 ;;
  esac
}
