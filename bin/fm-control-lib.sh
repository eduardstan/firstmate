...
fm_control_harness_supported() {  # <harness>
  case "${1-}" in
    claude|codex|opencode|pi|pi-signed|grok|kimi|prime-agent|cursor|muse) return 0 ;;  esac
  return 1
}
...
# bin/fm-spawn.sh refuses a --secondmate launch on either of them. The control plane
...
=======
    cursor|muse|prime-agent) [ "$kind" != secondmate ] || return 1 ;;
...
fm_control_interrupt_key() {  # <harness>
  case "${1-}" in
    claude|codex|opencode|pi|pi-signed|kimi|prime-agent|cursor|muse) printf 'Escape' ;;
    grok) printf 'C-c' ;;
    *) return 1 ;;
  esac
}
...
fm_control_interrupt_repeat() {  # <harness>
  case "${1-}" in
    opencode) printf '2' ;;
    claude|codex|pi|pi-signed|grok|kimi|prime-agent|cursor|muse) printf '1' ;;    *) return 1 ;;
  esac
}
...
# concatenate onto it. cursor and prime-agent do not restore the cancelled prompt, so neither
# needs a clear key. Prints the key or nothing;
# a harness with no verified mechanics returns nonzero, matching the tables
# above.
fm_control_interrupt_clear_key() {  # <harness>
  case "${1-}" in
    muse) printf 'C-u' ;;
    claude|codex|opencode|pi|pi-signed|grok|kimi|prime-agent|cursor) ;;    *) return 1 ;;
  esac
}
...
fm_control_interrupt_ack_source() {  # <harness>
  case "${1-}" in
    ...
    claude|codex|opencode|pi|pi-signed|grok|kimi|prime-agent|cursor) printf 'none' ;;    *) return 1 ;;
  esac
}
...
fm_control_exit_command() {  # <harness>
  case "${1-}" in
    ...
    codex|pi|pi-signed|prime-agent) printf '/quit' ;;    *) return 1 ;;
  esac
}
