...
# Which task kinds an adapter is verified to run. muse, gemini, rovo, and
# prime-agent are crewmate/scout adapters only: none has a primary supervision protocol,
...
fm_control_harness_supports_kind() {  # <harness> <kind>
  ...
  case "$harness" in
    muse|gemini|rovo|prime-agent) [ "$kind" != secondmate ] || return 1 ;;
  esac
  return 0
}
