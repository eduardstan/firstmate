...
fm_control_harness_supports_kind() {  # <harness> <kind>
  ...
  case "$harness" in
    muse|gemini|rovo|prime-agent|agy|devin) [ "$kind" != secondmate ] || return 1 ;;
  esac
  return 0
}
...
fm_control_interrupt_key() {  # <harness>
  case "${1-}" in
    claude|codex|opencode|pi|pi-signed|prime-agent|omp|kimi|cursor|gemini|muse|rovo|agy|devin) printf 'Escape' ;;
    grok) printf 'C-c' ;;
    *) return 1 ;;
  esac
}
...
fm_control_interrupt_arm_signal() {  # <harness>
  case "${1-}" in
    ...
    claude|codex|opencode|pi|pi-signed|prime-agent|omp|grok|kimi|cursor|gemini|muse|rovo|agy) ;;
    *) return 1 ;;
  esac
}
...
fm_control_interrupt_clear_key() {  # <harness>
  case "${1-}" in
    muse) printf 'C-u' ;;
    claude|codex|opencode|pi|pi-signed|prime-agent|omp|grok|kimi|cursor|gemini|rovo|agy|devin) ;;
    *) return 1 ;;
  esac
}
...
fm_control_interrupt_ack_source() {  # <harness>
  case "${1-}" in
    ...
    claude|codex|opencode|pi|pi-signed|prime-agent|omp|grok|kimi|cursor|gemini|rovo|agy|devin) printf 'none' ;;
    *) return 1 ;;
  esac
}
...
fm_control_exit_command() {  # <harness>
  case "${1-}" in
    ...
    codex|pi|pi-signed|prime-agent|omp|gemini|agy|devin) printf '/quit' ;;
    *) return 1 ;;
  esac
}
