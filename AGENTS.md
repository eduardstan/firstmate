...
The verified harnesses are `claude`, `codex`, `opencode`, `pi`, `pi-signed`, `prime-agent`, `grok`, `kimi`, and `cursor`, plus `muse`, `gemini`, and `rovo` for crewmates and scouts only; never dispatch on an unverified adapter.
