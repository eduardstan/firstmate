...
`bin/fm-cd-pretool-check.sh` supports all five harness-engine entry shapes used by the tracked adapters, with pi-signed and prime-agent sharing Pi's shape:
...
- Pi, pi-signed, and prime-agent send the exact command string through `--command <exact string>`.
...
- Pi, pi-signed, and prime-agent return `{block: true}` only when the checker exits 2.
...
| prime-agent | `.prime/agent/extensions/fm-primary-turnend-guard.ts` `tool_call` handler | Same CLI entry form and block shape as Pi, on the auto-discovered primary extension. |
