...
=======
The injection sits at the unconditional pre-launch export site, so it covers ship and scout spawns across `claude`, `codex`, `opencode`, `pi`, `pi-signed`, `prime-agent`, `grok`, `kimi`, `cursor`, and `muse`, plus Secondmate spawns across that same set except the deliberately crewmate-only `cursor`, `muse`, and `prime-agent` adapters.
>>>>>>> c949bc5 (no-mistakes(document): document prime-agent composer, control-plane, and spawn coverage)
