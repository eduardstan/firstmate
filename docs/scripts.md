...
| `fm-wake-drain.sh`       | Atomically drain queued watcher wakes, record the turn's wake kind in `state/.turn-wake-kind`, emit bounded best-effort status-event annotations, then assert watcher liveness |
