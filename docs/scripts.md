...
| `fm-brief.sh`            | Scaffold ship (explicit `--mode`), scout, secondmate-charter, Herdr-lab, and Prime Agent briefs |
...
| `fm-prime-agent-lib.sh`  | Single owner of prime-agent detached-daemon session facts: worker-abandoned liveness and cwd-scoped retirement for teardown and secondmate relaunch |
...
| `fm-wake-drain.sh`       | Atomically drain queued watcher wakes, record the turn's wake kind in `state/.turn-wake-kind`, emit bounded best-effort status-event annotations and a fleet-wide OPEN DECISIONS section, then assert supervision health |
