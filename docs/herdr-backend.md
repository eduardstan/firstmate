...
- OpenCode 1.18.4 can accept Enter while busy without clearing the composer.
  Both tmux and Herdr apply the same busy-queue fallback in their submit core
  (fm_tmux_submit_enter_core and fm_backend_herdr_send_text_submit): once the
  Enter-retry budget is spent, a proven-pending composer on an agent that is
  still mid-turn reports delivered (the Enter was queued), while an idle pane
  keeps the swallow.
