...
| --- | --- | --- |
| Run | Executes `bin/fm-session-start.sh` in the hook and lets its ordered digest land in model context before the first turn. | Claude, `codex exec`, Pi / pi-signed, prime-agent |
...
| prime-agent | Run | `.prime/agent/extensions/fm-primary-turnend-guard.ts` maps the same `session_start` reasons and `session_compact` onto wrapper sources and injects with `pi.sendMessage`, exactly as the Pi adapter does. | Auto-discovered from `.prime/agent/extensions/` with no trust gate, so the run tier needs no per-clone grant. |
...
The Pi-family adapters (Pi, pi-signed, and prime-agent) are the only ones that inject a message rather than hook stdout, so whatever they inject must carry operational provenance or the Ahoy skill would have to guess whether it was captain-authored.
