...
The generated task extension reports semantic busy and idle state through `agent_start` and `agent_end` and keeps `turn_end` as a notification.
Prime Agent is verified for local secondmate launches because its two primary supervision extensions load in the secondmate-shaped home.
The complete dated command transcript is [`docs/verification/prime-agent.md`](prime-agent.md).
`tests/fm-prime-agent-harness.test.sh`, `tests/fm-prime-watch-extension.test.sh`, `tests/fm-busy-adapter-wiring.test.sh`, and `tests/fm-spawn-dispatch-profile.test.sh` are the deterministic verification entry points.
