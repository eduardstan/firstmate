...
A zero/empty child return rechecks the home lock and beacon, and either attaches to a verified healthy successor, reports `watcher: cycle complete - supervision cycle ended without an actionable reason` and exits zero when the cycle demonstrably ran its supervision (the child declared a deliberate stand-down, or an attached peer ended having kept the beacon fresh), or emits `watcher: FAILED - cycle ended without an actionable reason` and exits nonzero.
A watched cycle that ended after doing its job is COMPLETED, not a false failure: an attached follower whose peer cycle ended cleanly, or an owned watcher that stood down for the singleton holder after running its polls, must not burn a primary turn. Only a genuine failure - a nonzero child exit, a confirmation timeout, or a live recorded watcher wedged behind a stale beacon - reports FAILED.
An attached arm follows verified identity-matched successors and reports the same completed-vs-failed outcome when that chain ends without one.
