...
`tests/fm-watch-triage.test.sh` covers live pane-child suppression until child exit, an idle version-named harness process that must not suppress anything, valid merge-wait pause cadence, and restoration of stale detection when the poll becomes invalid.
