...
Pi's `.pi/extensions/fm-primary-pi-watch.ts`, prime-agent's `.prime/agent/extensions/fm-primary-prime-watch.ts`, and OpenCode's `.opencode/plugins/fm-primary-watch-arm.js` own continuous re-arm after an actionable child close.
Each adapter starts the next arm before delivering the wake prompt, checks current session-lock ownership at launch, preserves one child or scheduled retry at a time, and applies bounded exponential retry after an unexpected or failed close.
A failed follow-up never cancels continuity restoration.
Pi and prime-agent same-process session replacement follows the generation-owner contract, stated in `.pi/extensions/fm-primary-pi-watch.ts` and restated in short in the prime-agent sibling.
...
After an actionable Pi, prime-agent, or OpenCode child close, the adapter starts and verifies one singleton successor before it delivers the original wake.
...
Claude's Stop hook starts the successor arm at the next Stop after the handling turn, rather than before notification as Pi, prime-agent, and OpenCode do.
...
A process in the pane's foreground process group counts as stale-pane activity while it exists, but an exited child is not evidence of progress and the next poll returns to normal wedge detection.
The scope is the foreground group, not the pane's descendants: a leftover the worker put in the background - an MCP stdio server, a detached dev server, a stray tail - never exits on its own, and counting it would mute the window for as long as it lives.
...
A live child quiets the immediate stale wake only. The `BUSY_TURN_MAX_SECS` no-completed-turn ceiling keeps accruing underneath it, so a tool call that hangs and never exits still wedge-escalates.
...
`tests/fm-pi-watch-extension.test.sh` checks Pi's first-cycle-or-explicit-repair tool metadata and ownership-based redundant-call no-ops, then simulates actionable and empty child closes against the actual Pi and OpenCode close handlers (a completed-cycle close is a routine non-failure retry that never consumes the failure budget), blocks prompt delivery to prove the successor launches first, verifies single-flight behavior, changes the session lock before close to prove ownership is rechecked, and hangs each successor arm to prove bounded fallback delivery includes the typed restoration failure.
The same suite covers ordinary same-process session replacement for `/new`, `/resume`, and `/fork`, same-instance shutdown-plus-start, stale prior-generation callbacks, repeated transitions with exactly one live cycle, disappearance of the shutting-down refusal after a valid replacement activates, and terminal quit still refusing late rearm.
`tests/fm-prime-watch-extension.test.sh` covers the prime-agent sibling extension's ownership-based redundant-call no-op, bounded retry and surfacing of a close that ends without an actionable reason, and one-generation-per-activation ownership of the arm child.
`tests/fm-watcher-lock.test.sh` covers verified-successor attach, the completed self-eviction cycle, the wedged-peer FAILED counterfactual, bounded and successor-linked lifecycle rows, and a SIGSTOP counterfactual that distinguishes a live PID from a stale beacon before classifying termination.
