...
An exact `closed` means the watched pull request was closed without merging, which is a normal outcome rather than a failure: the change may have landed in a successor pull request, so the wake exists for the first mate to reconcile where the work went.
