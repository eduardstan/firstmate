...
A live descendant of the recorded tmux pane process that is not the pane's own harness counts as that busy evidence while it lives, still under the same no-completed-turn ceiling; [`watcher-continuity.md`](watcher-continuity.md) owns the probe's scope, harness recognition, and bounds.
...
A finished crew whose latest status is `done:` and whose validated PR poll is still armed is the same declared external wait rather than a stale pane; [`watcher-continuity.md`](watcher-continuity.md) owns the exemption's two required halves.
...
A live-looking run on the crew's branch whose code identity cannot be established stops there at `unknown` rather than falling through to an older row or the fallbacks below, because both could report a still-running crew as terminal.
A terminal `passed` outcome is likewise `done` only once the forge confirms that run's PR merged, since the CI monitor can also complete on its own timeout with the PR still open.
...
Pi, prime-agent, and OpenCode verify session-lock ownership and launch one singleton successor from their child-close handlers before delivering an actionable wake prompt, with bounded exponential retry for failed restoration.
...
The shell scripts validate the JSON shape and verified harness/provider and harness/effort combinations, but they do not parse task intent, match natural-language rules, or own array selection.
...
That keeps spawn launch compatible across claude, codex, opencode, pi, pi-signed, prime-agent, grok, kimi, and muse while preserving the requested profile for later audit.
