...
FM_BACKEND_HERDR_COMPOSER_LINES=40  # herdr-only: tail lines scanned by composer-state guard/fallback paths; a harness footer plus a multi-line statusline can push the composer row past a 20-line window and read unknown; idle-baseline submit confirmation uses agent-state
