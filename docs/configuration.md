...
claude, codex, opencode, pi, pi-signed, grok, kimi, cursor, omp, and prime-agent are empirically verified for crewmate and local secondmate launches; gemini is verified for crewmate and scout launches only, and [README requirements](../README.md#requirements) own the set supported for the primary session.
...
prime-agent is verified for crewmate, scout, and local secondmate launches; its primary supervision protocol and detached-worker lifecycle are owned by [its adapter reference](../.agents/skills/harness-adapters/references/harness/prime-agent.md).
