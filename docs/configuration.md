...
An in-place restart needs no retirement at all: session-lock liveness asks prime-agent whether any client is still attached to the recorded worker, so a quit session's worker is reclaimable while an attached one still holds the lock.
For the same reason a quit pane classifies dead rather than alive - its reporter identity survives the agent - so secondmate recovery relaunches it instead of skipping it as live.
