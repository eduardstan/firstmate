...
It is not deterministic across the verified adapters: codex, grok, and prime-agent resume only from a session id printed at exit, opencode continues the most recent session for the cwd, muse resumes its last session or a named one, and claude, pi, pi-signed, and kimi have no verified pane-resume contract.
