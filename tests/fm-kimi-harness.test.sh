...
test_kimi_launch_then_send_is_verified() {
  ...
  [ "$launch" = "renice -n 10 -p \$\$ >/dev/null 2>&1 || exit 1; export COMPACT_ADVISER_DISABLE=1; env -u CURSOR_AGENT -u CURSOR_INVOKED_AS -u GEMINI_CLI -u PRIME_AGENT_CODING_AGENT_DIR -u PRIME_AGENT_INTERNAL_DAEMON_WORKER '$FAKEBIN_DIR/kimi' --model 'kimi-code/k3' --auto" ] \
...
test_kimi_falls_back_to_expanded_home_binary() {
  ...
  launch=$(cat "$CASE_DIR/launch.log")
  [ "$launch" = "renice -n 10 -p \$\$ >/dev/null 2>&1 || exit 1; export COMPACT_ADVISER_DISABLE=1; env -u CURSOR_AGENT -u CURSOR_INVOKED_AS -u GEMINI_CLI -u PRIME_AGENT_CODING_AGENT_DIR -u PRIME_AGENT_INTERNAL_DAEMON_WORKER '$fallback' --auto" ] \
