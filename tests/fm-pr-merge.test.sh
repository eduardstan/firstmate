...
make_case() {
  ...
  : > "$case_dir/gh.log"
  # The fork's done gate proves a named forge head against the recorded
  # worktree before arming the poll, so give every GitHub fixture the same
  # pushed-looking clean copy that a real ship task has.
...
test_away_grant_and_yolo_and_hold_for_return() {
  ...
  run_pr_merge "$case_dir" task-x1 "$url" \
    > "$case_dir/stdout" 2> "$case_dir/stderr" \
    || fail "away-held: a green merge under the away record should succeed"
  ...
  assert_grep "merge landed: task-x1 $url away" "$case_dir/state/.wake-queue" \
    "away-held: the durable outcome did not tag away"
  ...
  run_pr_merge "$case_dir" task-x1 "$url" --attended-override \
    > "$case_dir/stdout" 2> "$case_dir/stderr" \
    || fail "away-held-override: a green merge should succeed"
  assert_logged_gh_merge "$case_dir" 83 example/repo --squash
  assert_grep "merge landed: task-x1 $url away" "$case_dir/state/.wake-queue" \
    "away-held-override: the durable outcome did not tag away"
  ...
  pass "away merges require a live words record, and --attended-override does not skip that"
}
...
SH
  ...
  assert_equals 'merge task-x1 when green' "$(cat "$case_dir/away-words-at-merge" 2>/dev/null || true)" \
    "away-archive-at-merge: the words this merge read were not still standing at the forge call"
  assert_grep "merge landed: task-x1 https://github.com/example/repo/pull/71 away" \
    "$case_dir/state/.wake-queue" \
    "away-archive-at-merge: the landed merge was not recorded under the away posture it read"
...
"$CONTRACT" enter --words 'merge task-other when green'
SH
  )
  export FM_TEST_AWAY_MUTATE_AT_MERGE="$mutate"
  set +e
  ...
  assert_equals 'merge task-x1 when green' "$(cat "$case_dir/away-words-at-merge" 2>/dev/null || true)" \
    "away-revoke-at-merge: the words were changed inside the merge's critical section"
  pass "no away-record archive or words replacement lands between the authority read and the merge"
}

# The same serialization from the other side. A record change that wins the
# race lands BEFORE the in-lock authority read, and the merge then refuses an
# unreadable record rather than merging on the record the opening partition saw.
