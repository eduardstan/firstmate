...
test_active_dispatch_profile_allows_raw_launch_command() {
  ...
  launch=$(cat "$LAUNCH_LOG")
  [ "$launch" = 'renice -n 10 -p $$ >/dev/null 2>&1 || exit 1; export COMPACT_ADVISER_DISABLE=1; custom-agent --flag' ] \
...
test_nice_override_keeps_raw_launch_command_verbatim() {
  ...
  launch=$(cat "$LAUNCH_LOG")
  [ "$launch" = "export COMPACT_ADVISER_DISABLE=1; custom-agent --flag" ] \
