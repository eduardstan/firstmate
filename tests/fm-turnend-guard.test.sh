...
run_hook() {
  ...
  home=$(cd "$dir" && pwd)
  # Do not let the agent running this test outrank the intended Claude marker.
...
test_hook_blocks_from_fm_home_state() {
  ...
  out=$(printf '{"stop_hook_active":false}' | env -u PI_CODING_AGENT -u PRIME_AGENT_CODING_AGENT_DIR -u PRIME_AGENT_INTERNAL_DAEMON_WORKER -u FM_PI_HARNESS -u GROK_AGENT CLAUDECODE=1 FM_HOME="$home" bash "$dir/bin/fm-turnend-guard.sh" 2>&1); status=$?
...
test_hook_uses_state_override() {
  ...
  : > "$state/task1.meta"
  out=$(printf '{"stop_hook_active":false}' | env -u PI_CODING_AGENT -u PRIME_AGENT_CODING_AGENT_DIR -u PRIME_AGENT_INTERNAL_DAEMON_WORKER -u FM_PI_HARNESS -u GROK_AGENT CLAUDECODE=1 FM_HOME="$home" FM_STATE_OVERRIDE="$state" bash "$dir/bin/fm-turnend-guard.sh" 2>&1); status=$?
...
run_hook_claude() {
  local dir=$1 stop_active=$2 home
  home=$(cd "$dir" && pwd)
  printf '{"stop_hook_active":%s,"session_id":"sess-claude-mode"}' "$stop_active" | env -u PI_CODING_AGENT -u PRIME_AGENT_CODING_AGENT_DIR -u PRIME_AGENT_INTERNAL_DAEMON_WORKER -u FM_PI_HARNESS -u GROK_AGENT CLAUDECODE=1 FM_HOME="$home" bash "$dir/bin/fm-turnend-guard.sh" --claude 2>&1
}
  ...
  (
    printf '{"stop_hook_active":true,"session_id":"sess-claude-mode"}' \
      | PATH="$fakebin:$PATH" \
        ...
        env -u PI_CODING_AGENT -u PRIME_AGENT_CODING_AGENT_DIR -u PRIME_AGENT_INTERNAL_DAEMON_WORKER -u FM_PI_HARNESS -u GROK_AGENT CLAUDECODE=1 FM_HOME="$dir" bash "$dir/bin/fm-turnend-guard.sh" --claude \
          > "$guard_out" 2>&1
    printf '%s\n' "$?" > "$guard_status"
  ) &
  guard_pid=$!
