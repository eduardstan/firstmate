...
This slice detects Prime Agent from the explicit `FM_PI_HARNESS=prime-agent` launch marker only, so a home that does not set it keeps the Pi-family behavior it had before, and `prime-agent` remains selectable as a `config/crew-harness` or `config/secondmate-harness` value.
Ambient auto-detection of a Prime Agent primary arrives with the launch and supervision slices that give the value a launch template, a supervision model, and a supervision snippet.
