...
- A verified primary agent harness: Claude Code, Grok, Pi, `pi-signed`, prime-agent, Codex, or OpenCode.
...
Codex, OpenCode, and prime-agent are also verified and supported as primary harnesses; Codex uses bounded foreground checkpoints, OpenCode uses a TUI plugin, and prime-agent uses its own pair of tracked extensions under `.prime/agent/extensions/`, so all three carry more harness-specific supervision tradeoffs than the three co-primaries.
...
- [docs/supervision-protocols/](docs/supervision-protocols/) - rendered primary-harness watcher protocols for Claude, Codex, OpenCode, Pi and `pi-signed`, prime-agent, Grok, and unknown harness fallback.
