// Firstmate primary watcher bridge for prime-agent.
//
// The prime-agent sibling of `.pi/extensions/fm-primary-pi-watch.ts`, with the
// same session-generation ownership contract stated once there and repeated in
// short here because it is what keeps a stale callback from rearming:
// prime-agent emits `session_shutdown` for ordinary same-process replacements
// (/new, /resume, /fork, reload) as well as terminal quit, so this extension
// binds one generation per session activation. Only the active live generation
// may start, stop, rearm, or clear the arm child. Replacement `session_start`
// activates a new live generation. Terminal quit leaves the final generation
// stopped.
//
// Two deliberate differences from the Pi file, both because prime-agent's
// surface genuinely differs rather than for convenience:
//   - No Calm presentation integration. prime-agent's ExtensionAPI has no
//     `registerEntryRenderer` (pi 0.83.0 does), and it discovers only
//     `.prime/agent/extensions/`, never `.pi/`, so the tracked Calm extension
//     is invisible to it and there is no Calm surface to coordinate with.
//   - No custom tool rendering for the same reason; the tool registers with
//     prime-agent's stock rendering.
...
const marker = `${state}/.prime-watch-extension-loaded`;
const extensionVersion = `sha256:${createHash("sha256").update(readFileSync(extensionFile)).digest("hex")}`;
const retryBaseMs = positiveInteger("FM_WATCH_REARM_RETRY_BASE_MS", 250);
const retryMaxMs = positiveInteger("FM_WATCH_REARM_RETRY_MAX_MS", 4000);
const retryLimit = positiveInteger("FM_WATCH_REARM_RETRY_LIMIT", 5);
const armReadyTimeoutMs = positiveInteger(
  "FM_PRIME_ARM_READY_TIMEOUT_MS",
  process.platform === "win32" ? 35000 : 12000,
);
const armRetireTimeoutMs = positiveInteger("FM_WATCH_ARM_RETIRE_TIMEOUT_MS", 1000);
const repairOnlyHint = "call fm_watch_arm_prime again only after a later notification says the cycle is missing, failed, or unhealthy";
const shuttingDownMessage = "watcher: not armed - the prime-agent session is shutting down";
...
function classifyClose(stdout: string, stderr: string, code: number | null, signal: NodeJS.Signals | null): CloseClassification {
  ...
  if (healthy) {
    return {
      kind: "failure",
      message: `watcher: FAILED - the prime-agent extension arm child found an external healthy watcher instead of owning wake delivery\n${healthy}`,
    };
  }
  ...
  if (signal) {
    return {
      kind: "failure",
      message: `watcher: FAILED - the prime-agent extension arm child ended from ${signal}${combined ? `\n${combined}` : ""}`,
    };
  }
  if (code && code !== 0) {
    return {
  ...
  return {
    kind: "failure",
    message: "watcher: FAILED - the prime-agent extension arm cycle ended without an actionable reason",
  };
}
...
export default function (pi: ExtensionAPI) {
  ...
  function surfaceFailure(owner: SessionGeneration, message: string): void {
    void sendWake(owner, message).catch(() => {
      // Delivery errors belong to prime-agent; continuity never waits on prompting.
    });
  }
  ...
  async function restoreAfterActionableClose(owner: SessionGeneration, predecessorArmPid: string): Promise<string> {
    ...
    for (let attempt = 0; attempt <= retryLimit; attempt += 1) {
      ...
      if (replacement.ok) {
        failure = "watcher: FAILED - the prime-agent extension could not verify a ready successor watcher";
        if (!(await retireArm(successorChild))) {
          return `${failure}\nwatcher: FAILED - the prime-agent extension could not restore watcher continuity because the unready successor arm did not exit within ${armRetireTimeoutMs}ms`;
        }
      } else {
        failure = /(?:read-only|no live session)/.test(replacement.message)
          ? `watcher: FAILED - the prime-agent extension cannot restore continuity because this session no longer owns the lock\n${replacement.message}`
          : `watcher: FAILED - the prime-agent extension could not start the successor watcher cycle\n${replacement.message}`;
        if (/(?:read-only|no live session)/.test(replacement.message)) break;
      }
      if (attempt === retryLimit) break;
      await waitForRetry(attempt + 1);
    }
    return `${failure}\nwatcher: FAILED - the prime-agent extension could not restore watcher continuity after ${retryLimit} retries`;
  }
  ...
  function scheduleRetry(owner: SessionGeneration, message: string, predecessorArmPid: string): void {
    ...
    if (ownership !== "owned") {
      surfaceFailure(owner, `watcher: FAILED - the prime-agent extension cannot restore continuity because this session no longer owns the lock\n${message}`);
      return;
    }
    owner.retryFailures += 1;
    if (owner.retryFailures > retryLimit) {
      surfaceFailure(owner, `watcher: FAILED - the prime-agent extension could not restore watcher continuity after ${retryLimit} retries\n${message}`);
      return;
    }
    const timer = setTimeout(() => {
      ...
      if (!result.ok) {
        surfaceFailure(owner, `watcher: FAILED - the prime-agent extension could not launch a continuity retry\n${result.message}`);
      }
  ...
  function startArm(owner: SessionGeneration, predecessorArmPid = ""): ArmResult {
    ...
    if (ownership === "missing") {
      return {
        ok: false,
        message: "watcher: not armed - no live session holds the lock; run bin/fm-session-start.sh to reclaim it, then call fm_watch_arm_prime to re-arm",
      };
    }
    markLoaded();
    if (owner.child) {
      return {
        ok: true,
        message: `watcher: unchanged - the prime-agent extension already owns an arm child; no manual re-arm needed; ${repairOnlyHint}`,
      };
    }
    if (owner.retryTimer) {
      return {
        ok: true,
        message: `watcher: unchanged - the prime-agent extension already owns a scheduled continuity retry; no manual re-arm needed; ${repairOnlyHint}`,
      };
    }
    const id = ++owner.seq;
    const env = {
    ...
    armChild.on("error", (error: Error) => {
      ...
      if (!generationIsLive(owner)) return;
      if (owner.restoring) return;
      scheduleRetry(owner, `watcher: FAILED - the prime-agent extension arm child ${id} failed: ${error.message}`, String(armChild.pid ?? ""));
    });
    return {
      ok: true,
      message: `watcher: started prime-agent extension arm child ${id}; future ordinary re-arms are automatic; ${repairOnlyHint}`,
    };
  }
  ...
  pi.registerTool?.({
    name: "fm_watch_arm_prime",
    label: "Arm firstmate watcher",
    description: "Start the first required prime-agent watcher cycle, or repair one only after a notification says the cycle is missing, failed, or unhealthy. Do not call after ordinary work or ordinary notifications; the extension re-arms automatically. Never run bin/fm-watch-arm.sh through bash.",
    promptSnippet: "Start the first required prime-agent watcher cycle or repair a cycle reported missing, failed, or unhealthy; ordinary re-arming is automatic.",
    promptGuidelines: [
